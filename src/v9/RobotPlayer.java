...
public strictfp class RobotPlayer {
    ...
    public static void move() throws GameActionException {
        // attack before we move anywhere
        ...
        // getting a location we want to move to (after running combat movement?)
        ...
        // we have a location that we want to move to, now we want to handle the different cases
        // for whether or not we can move there
        if(target != null) {
            // first check if we don't have the flag, then we know we are moving as normal
            // so check for obstructions, try to get around them, and try to remove them in that
            // order
            if(!rc.hasFlag()) {
                ...
                if(rc.canMove(towards)) {
                    ...
                    if(Utils.isNearEnemyFlag(25)) {
                        ...
                        // there is an obstruction in the direct path, check the cw + ccw cells
                        ...
                        // check if there are obstructions in side blocks, try to move to them, if 
                        // can't move, try to remove obstructions
                        if(Utils.isValidMapLocation(cwTarget) && rc.canMove(cw)) {
                            Pathfinding.initTurn();
                            Pathfinding.move(cwTarget); 
                        } else if(Utils.isValidMapLocation(ccwTarget) && rc.canMove(ccw)) {
                            Pathfinding.initTurn();
                            Pathfinding.move(ccwTarget); 
                        } else {
                            // both sides are obstructed, we're going to check for water to fill up
                            if(rc.canFill(target)) rc.fill(target);
                            else if(rc.canFill(cwTarget)) rc.fill(cwTarget);
                            else if(rc.canFill(ccwTarget)) rc.fill(ccwTarget);
                            else {
                                // IDK BRO CANT MOVE CANT FILL BRUH JUST DIE
                                Pathfinding.initTurn();
                                Pathfinding.move(target);             
                            }
                        }
                            ...
                            if(rc.hasFlag()) {
                                if(rc.isActionReady()) {
                                    ...
                                    if(rc.canFill(target)) {
                                        ...
                                        Pathfinding.initTurn();
                                        Pathfinding.move(target);             
                                    } else if(rc.canFill(cwTarget)) {
                                        rc.fill(cwTarget);
                                        Pathfinding.initTurn();
                                        Pathfinding.move(cwTarget);             
                                    } else if(rc.canFill(ccwTarget)) { 
                                        rc.fill(ccwTarget); 
                                        Pathfinding.initTurn();
                                        Pathfinding.move(ccwTarget);             
                                    } else {
